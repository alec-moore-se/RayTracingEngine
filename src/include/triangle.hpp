...
public:
  ...
  bool intersect_with_backface(const ray &r, interval ray_t, hit_rec &rec,
                               double &det, double &i_det, vec3 &E1, vec3 &E2,
                               vec3 &t, vec3 &p, vec3 &q) {
  ...
  vec3 barycentric_coords(const point3 &P) {
    vec3 bary;
    bary[0] = (dot_product(N(*this), Na(*this, P))) / (areaTot() * areaTot());
    bary[1] = (dot_product(N(*this), Nb(*this, P))) / (areaTot() * areaTot());
    bary[2] = (dot_product(N(*this), Nc(*this, P))) / (areaTot() * areaTot());
    return bary;
  }
